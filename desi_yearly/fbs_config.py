...
def get_scheduler(for_simulation=False) -> tuple[int, CoreScheduler]:
    ...
    sum_desi = (desi_y2 + desi_y4) / 2.0
    desi_fp_y2 = Footprint(SURVEY_START_MJD, sun_ra_start)
    desi_fp_y4 = Footprint(SURVEY_START_MJD, sun_ra_start)
    for bandname in "ugriz":
        desi_fp_y2.set_footprint(bandname, desi_y2)
        desi_fp_y4.set_footprint(bandname, desi_y4)

    desi = lsst_surveys.gen_desi_surveys(
        footprints=desi_fp_y2,
        nside=nside,
        band1s=["u", "g"],
        dark_only=["u", "g"],
        exptime=exptime,

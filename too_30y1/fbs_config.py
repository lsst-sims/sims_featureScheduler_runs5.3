...
class ScaleTimeEarlyDetailer(detailers.BaseDetailer):
        ...
        if conditions.night <= self.end_night:
            ...
            if in_band.size > 0:
                for obs in observation_array[in_band]:
                    n_replace = np.ceil(obs["exptime"] / self.max_time)
                    ...
                    replacement["exptime"] = self.max_time
...
def get_scheduler(for_simulation=False) -> tuple[int, CoreScheduler]:
    ...
    for band in band_expt:
        too_detailers.append(ScaleTimeEarlyDetailer(bandname=band,
                                                    max_time=band_expt[band],
                                                    end_night=730))
